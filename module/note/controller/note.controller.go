...
func GetAllNotes(c *fiber.Ctx) error {
	...
	if len(notes) == 0 {
		return c.Status(200).JSON(fiber.Map{"status": "error", "message": "No notes present", "data": nil})
	}

	return c.Status(200).JSON(fiber.Map{"status": "success", "message": "Notes fetched successfully", "data": notes})
}
...
func UpdateNote(c *fiber.Ctx) error {
    ...
    note.Content = updateNoteData.Content
